...
def gradient_descent(X,y,w0,lam):
    ...
    max_its = 5000
    alpha = 10**(-2)
    w_reg = np.empty(w.shape)
    for k in range(max_its):
        # compute gradient
        one = 1/(1 + my_exp(-np.dot(X,w))) - y
        two = one + y
        three = np.ones(y.shape) - 1/(1 + my_exp(-np.dot(X,w)))
        regsum = 2 * np.sum(one*two*three*X, axis=0)
        w_reg[:] = w[:]
        w_reg[0] = 0
        grad = regsum[np.newaxis,:].T + 2*lam*w_reg  
